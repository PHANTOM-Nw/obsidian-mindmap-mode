...
匹配的是你在卡片上看到的文字：`**bold** text` 能被"bold text"搜到，`[[note|Label]]` 只能按"Label"搜到、按"note"搜不到，公式则按 TeX 源码匹配。跳到折叠分支里的命中会自动展开，关闭查找条时再折回去 —— 搜索期间你自己手动展开的分支会保留。这一版只给命中的整张卡片加高亮环，不高亮卡内的具体子串；段落、代码块、表格这些内容卡片也不参与搜索。
