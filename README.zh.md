...
最后一组是 **Shortcuts**：导图响应的每个按键各占一行，显示它当前绑的键。**Record** 会把你按下的下一个键记为新绑定 —— `Esc` 放弃，`Backspace` 或 `Delete` 让这个动作变成未绑定；只有改过的行才会出现那个重置箭头，点一下就恢复默认。两个动作绑到同一个键时，两行都会提示，并且由列在上面的那个响应。组末尾的 **Restore all defaults** 一次清掉所有改动。只有你改过的部分会被存下来，所以以后版本调整了某个默认键，你这边也会跟着变。
