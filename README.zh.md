...
标题或列表项下面的 `: ` 行会附在该节点下方，以灰色文字和左侧竖线显示，不再生成独立分支。连续的注释行保留换行，单独一行 `:` 表示注释内的空行；长行按 **Maximum card width** 自动折行，不改写源文件。
...
右键标题选择 **Add annotation / Edit annotation**，或双击灰色注释，即可编辑。编辑框里直接按 Enter 换行、留空行分段，无需输入冒号；点击 **Save** 或按 `Ctrl/Cmd+Enter` 保存，清空后保存会删除注释。标题重命名和注释编辑互不覆盖，注释随所属节点移动，支持撤销/重做。若编辑期间原注释被外部修改，保存会提示重新打开，保留当前草稿供复制。

注释在节点折叠时仍显示；关闭 **Show note content** 也不会隐藏注释。在 **Settings → Mindmap Mode → Appearance → Inline annotations** 关闭此功能，可恢复原有正文卡片显示。围栏代码、缩进代码及 `:word` 不被识别为注释；需要字面冒号时可写 `\: text`。被节点来源或标题深度设置排除的结构，其注释保留为普通正文。导图搜索目前仍只搜索节点标题。
