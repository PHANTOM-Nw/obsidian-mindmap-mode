...
- 节点注释：标题或列表项下面以 `: ` 开头的行不再单独成卡，而是贴在该节点卡片下方，以灰色文字和左侧竖线显示。连续的 `: ` 行保留换行，单独一行 `:` 表示注释里的空行，过长的行按卡片最大宽度折行。注释在节点折叠时仍然可见，也不受 **Show note content** 影响。<br>Node annotations: a line beginning with `: ` under a heading or list item no longer becomes a card of its own — it hangs under that node's card in muted text behind a vertical rule. Consecutive `: ` lines keep their line breaks, a lone `:` is a blank line inside the annotation, and a long line wraps at the card's maximum width. An annotation stays visible when its node is folded, and does not depend on **Show note content**.
