...
on a card, dragging a node to a new position among its siblings, and the
buttons that grow the tree.
...
- A link in a *title* opens too: click [[Mindmap Demo]] here and the note opens;
  press the same link and drag, and the node moves as it always did
- [[Mindmap Demo|A title that is nothing but a link]] — still draggable, because
  only a press that never became a drag counts as a click

### A link in a heading: [obsidian.md](https://obsidian.md)

Heading cards, list items and the root all follow a link on a plain click, and
`Ctrl`/`Cmd`+click still opens it in a new pane.
...
the "Links on a card" / "Reordering" row cannot — those are first-level
